...
import { formatCurrency, getClients, getPayments, getSettings } from "@/lib/storage";
...
function ReportsPage() {
  ...
  const monthly = useMemo(() => {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      arr.push({ month: d.toLocaleString("en", { month: "short" }), earnings: total });
    }
    return arr;
  }, [payments]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <Card className="rounded-2xl border-0 bg-gradient-brand-strong p-5 text-white shadow-glow">
          ...
          <div className="mt-2 text-3xl font-extrabold">{formatCurrency(total, settings.currency)}</div>
        </Card>
        <Card className="rounded-2xl border-0 p-5 shadow-soft">
          ...
          <div className="mt-2 text-3xl font-extrabold">{formatCurrency(byType[0].value, settings.currency)}</div>
        </Card>
        <Card className="rounded-2xl border-0 p-5 shadow-soft">
          <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">From annual plans</div>
          <div className="mt-2 text-3xl font-extrabold">{formatCurrency(byType[1].value, settings.currency)}</div>
        </Card>
      </div>
